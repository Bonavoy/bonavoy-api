...
export default {
  Query: {
    getSpotOfInterestRecommendations: async (
      ...
      { ctx, req, res, dataSources }: { ctx: TokenDecoded; req: Request; res: Response; dataSources: any },
    ) => {
      //WRITE TYPES
      const { input } = args
      const options = {
      ...
      const spotOfInterestRecommendations = await dataSources.foursquareAPI.getSpotsOfInterest(options)
